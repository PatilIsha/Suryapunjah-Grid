...
5. **Logo** — `public/img/logo-compact.svg` (header/footer) and `logo.svg` (full lockup with tagline) are SVG recreations of your logo. Drop your original artwork in as `public/img/logo.png` and update the two `<img src>` references if you prefer the original file.
